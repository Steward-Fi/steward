...
| `REDIS_URL` | Redis for rate limiting and token/spend state. Production adapter swap/earn/bridge daily-cap checks deny without available Redis; the production proxy also fails closed unless its explicit soft-fail override is enabled. |
