...
STEWARD_DB_PLATFORM_PASSWORD="${STEWARD_DB_PLATFORM_PASSWORD:-$(env_get STEWARD_DB_PLATFORM_PASSWORD)}"
[[ -n "${STEWARD_DB_PLATFORM_PASSWORD}" ]] || STEWARD_DB_PLATFORM_PASSWORD="$(openssl rand -hex 32)"
...
EXPLICIT_PLATFORM_DATABASE_URL="${STEWARD_PLATFORM_DATABASE_URL-}"
...
STEWARD_PLATFORM_DATABASE_URL="${EXPLICIT_PLATFORM_DATABASE_URL:-$(env_get STEWARD_PLATFORM_DATABASE_URL)}"
...
if [[ -n "${EXPLICIT_DATABASE_URL}" || -n "${EXPLICIT_ADMIN_DATABASE_URL}" || -n "${EXPLICIT_MIGRATION_DATABASE_URL}" || -n "${EXPLICIT_PLATFORM_DATABASE_URL}" ]]; then
  if [[ -z "${EXPLICIT_DATABASE_URL}" || -z "${EXPLICIT_ADMIN_DATABASE_URL}" || -z "${EXPLICIT_MIGRATION_DATABASE_URL}" || -z "${EXPLICIT_PLATFORM_DATABASE_URL}" ]]; then
    echo "❌ External PostgreSQL requires DATABASE_URL, STEWARD_ADMIN_DATABASE_URL, MIGRATION_DATABASE_URL, and STEWARD_PLATFORM_DATABASE_URL together"
    exit 1
  fi
  ...
  if [[ -z "${DATABASE_URL}" || -z "${STEWARD_ADMIN_DATABASE_URL}" || -z "${MIGRATION_DATABASE_URL}" || -z "${STEWARD_PLATFORM_DATABASE_URL}" ]]; then
...
elif [[ -n "${DATABASE_URL}" || -n "${STEWARD_ADMIN_DATABASE_URL}" || -n "${MIGRATION_DATABASE_URL}" || -n "${STEWARD_PLATFORM_DATABASE_URL}" ]]; then
  if [[ -z "${DATABASE_URL}" || -z "${STEWARD_ADMIN_DATABASE_URL}" || -z "${MIGRATION_DATABASE_URL}" || -z "${STEWARD_PLATFORM_DATABASE_URL}" ]]; then
  ...
  STEWARD_PLATFORM_DATABASE_URL="postgresql://steward_platform:${STEWARD_DB_PLATFORM_PASSWORD}@steward-db:5432/steward"
  STEWARD_ALLOW_INSECURE_DB=true
  STEWARD_BOOTSTRAP_SET_ROLE_PASSWORDS=true
fi
...
for env_name in STEWARD_MASTER_PASSWORD STEWARD_JWT_SECRET STEWARD_KDF_SALT \
  ...
  POSTGRES_PASSWORD STEWARD_DB_APP_PASSWORD STEWARD_DB_MIGRATION_PASSWORD STEWARD_DB_PLATFORM_PASSWORD \
  DATABASE_URL STEWARD_ADMIN_DATABASE_URL MIGRATION_DATABASE_URL STEWARD_PLATFORM_DATABASE_URL REDIS_URL RPC_URL SOLANA_RPC_URL; do
  env_value="${!env_name-}"
  if [[ "${env_value}" == *$'\n'* || "${env_value}" == *$'\r'* ]]; then
...
STEWARD_DB_PLATFORM_PASSWORD=${STEWARD_DB_PLATFORM_PASSWORD}
...
STEWARD_PLATFORM_DATABASE_URL=${STEWARD_PLATFORM_DATABASE_URL:-}
