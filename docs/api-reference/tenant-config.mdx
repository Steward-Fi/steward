...
App clients can also have backend-only app secrets for Privy-style REST API authentication. The app ID is stable and public in the form `<tenantId>/<clientId>`. The app secret is returned only once when rotated or created.
...
Steward stores an opaque `RelayState` and `InResponseTo` request ID, redirects to the tenant IdP, then the ACS route validates the signed SAML response/assertion against the tenant IdP certs, SP Entity ID, ACS URL, request ID, assertion timestamps, verified email domain, and assertion replay table. Successful ACS returns the app to `redirect_uri` with a short-lived exchange `code`; the app redeems it through `POST /auth/oauth/exchange`.
