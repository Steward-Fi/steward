...
`authorizationDetails` is retained for schema compatibility. This route supports
only an omitted or empty array and rejects non-empty values because it has no
multi-approver enforcement path.
