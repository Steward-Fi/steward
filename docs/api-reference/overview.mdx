...
Both Bun and Workers enforce the global per-client request limit through Redis
in production, so replicas and restarts share one budget. A guaranteed
single-instance Bun deployment may explicitly acknowledge a restart-resetting
memory limiter; development and test use bounded memory by default. Sensitive
wallet routes can also enforce Redis-backed per-agent limits via
