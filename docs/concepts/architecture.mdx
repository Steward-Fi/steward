...
flowchart LR
    ...
    MP["Master password + deployment KDF salt"] --> SK["Signing-vault KDF root"]
    MP --> SV["Secret-vault KDF root"]
    SK --> WK["Random per-record wallet keys"]
    SV --> CK["Random per-record secret keys"]
    TC["Tenant + record context"] -->|"AES-GCM AAD"| WK
    TC -->|"AES-GCM AAD"| CK
    WK --> W1
    WK --> W2
    CK --> S1
...
- **Tenants** are isolated in the service layer through tenant-scoped authorization, filtered queries, and composite foreign keys. PostgreSQL production deployments add installed RLS policies, separate application/platform roles, explicit `ENABLE` + `FORCE ROW LEVEL SECURITY` activation, and a startup/readiness catalog check that fails closed on role or policy drift. Embedded PGLite/local mode does not provide that PostgreSQL RLS boundary. See the [database RLS rollout](../security/database-rls-rollout.mdx).
- **Agents** authenticate with JWTs scoped to their tenant and agent ID
- **Secrets and wallet keys** use domain-separated roots derived from the master password and deployment KDF salt. Each ciphertext derives a fresh per-record key from a random salt and authenticates its tenant/agent/name/version context as AES-GCM AAD; Steward does not create a separate stored key per tenant. See the [key-rotation runbook](../runbooks/key-rotation.md) for legacy-root migration and rotation requirements.
