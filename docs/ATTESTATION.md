...
Pull-request CI builds `@stwd/attestation` and runs `bun test scripts/__tests__/check-attestation.test.ts` to exercise the trust-gate configuration without network access or release credentials. That fixture test does not attest a deployment. A provisioned release or operator pipeline must run the command above with the real endpoint, verifier, and trusted-key fingerprints.
