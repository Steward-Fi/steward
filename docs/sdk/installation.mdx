...
Package versions are released independently. Check the package registry or the corresponding
`packages/*/package.json` manifest for the current version rather than copying a version from this
guide.

The tag-driven release workflow publishes `@stwd/sdk`, `@stwd/react`, and
`@stwd/eliza-plugin`. Other non-private packages are not published by that workflow.
