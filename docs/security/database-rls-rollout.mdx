...
and bootstrap-function surface. Migration `0112_tenant_rls_policy_manifest`
then verifies it against a checked-in, catalog-independent semantic manifest;
both intentionally leave RLS disabled. Routine
...
Production uses three distinct connection authorities. Never reuse the routine
application `DATABASE_URL` for bootstrap or migrations:

```bash
export STEWARD_ADMIN_DATABASE_URL='postgresql://bootstrap-admin:...@host/steward?sslmode=verify-full'
export DATABASE_URL='postgresql://steward_app:...@host/steward?sslmode=verify-full' # runtime only
```

For a generated external-database environment, `steward init` requires the
three authorities together and never derives one from another:

```bash
steward init --database-url "$DATABASE_URL" \
  --admin-database-url "$STEWARD_ADMIN_DATABASE_URL" \
  --migration-database-url "$MIGRATION_DATABASE_URL"
```

The shipped Compose files likewise reject a missing member of this set instead
of falling back to their bundled Postgres service. For external PostgreSQL,
create the `steward_app` and `steward_migrator` login credentials in the
provider before starting Compose; bootstrap preserves those passwords
(`STEWARD_BOOTSTRAP_SET_ROLE_PASSWORDS=false`). The generated bundled profile
sets that switch to `true` because it owns and creates both credentials.

All URLs must be host-bearing TCP URLs; socket/hostless URLs do not reliably
preserve substituted login credentials in operator tooling. The API
now refuses production startup before composing the app when
`SKIP_MIGRATIONS` is absent, and then verifies the live `DATABASE_URL` role and
complete policy catalog before bootstrap or traffic.

The fresh-database order is executable and strict:

1. Apply core migrations through `0112` with the initial schema-owning admin.
2. Run `rls-bootstrap.sql` as that admin to create roles and transfer ownership.
3. Apply enabled-plugin migrations with the new migration-role credential.
4. Activate as the admin, naming both the app and migration roles.
5. Start API/proxy/Workers with only the app-role `DATABASE_URL` and
   `SKIP_MIGRATIONS=1`.

```bash
DATABASE_URL= MIGRATION_DATABASE_URL="$STEWARD_ADMIN_DATABASE_URL" \
  bun --cwd packages/api run migrate:production:core
```

Create/finalize roles (the invoking administrator needs `CREATEROLE` and schema
ownership):

```bash
psql "$STEWARD_ADMIN_DATABASE_URL" \
...
Set the new migration-role credential, then apply exactly the plugins enabled
for this deployment through the same resolver used by the application:

```bash
export MIGRATION_DATABASE_URL='postgresql://steward_migrator:...@host/steward?sslmode=verify-full'
...
Rollback is an emergency operator action using the bootstrap administrator,
never a permission
...
```bash
psql "$STEWARD_ADMIN_DATABASE_URL" -f scripts/postgres/rls-rollback.sql
```
