...
- platform routes intentionally operate across tenants and must be decomposed
  into tenant transactions (tenant creation must use a narrow bootstrap API);
- the standalone proxy entry and the retention, webhook-retry, provider
  reservation, and transaction-receipt background schedulers need the same
  transaction propagation and real-Postgres route/job proofs.
