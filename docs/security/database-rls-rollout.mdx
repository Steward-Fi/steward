...
description: "Pre-activation SEC-169 transaction groundwork and the remaining rollout gates"
---
...
tenant authority boundary and schema inventory are groundwork; they do
not justify enabling a partial policy set.

## Non-forgeable context boundary

The application composition root creates one `createTenantRlsAuthority`
boundary with verifier callbacks over its authenticated-principal and
scheduled-job provenance types. The returned `issuer` capability is retained
only by verified JWT/API-key/app-secret middleware and named job runners. Tenant
data-access code receives the separate `transactions` capability plus an
already-issued context;
it cannot mint a context from a route parameter, header, JSON field, or provider
payload. A context issued by another authority instance is rejected.
The combined boundary and issuer must not be exported from a general-purpose
service module.

Background workers must first enumerate tenant ids using a maintenance path,
then use the retained issuer with scheduler-owned provenance and run one
transaction per tenant. The verifier must reject user-constructed job inputs;
a job name string alone is not provenance.
...
release until the caller's outer transaction ended. Each authority tracks its
contexts by object identity, so cloning a context or presenting one from a
different authority cannot change tenant authority.
