...
The personal-lifecycle rollout has an explicit split-role sequence. After
`0112` and role bootstrap, run the privileged 0113 handoff, apply unchanged
`0113` as the restricted migration role, then run the privileged wrapper
upgrade before restricted `0114+`. Finally rerun `rls-bootstrap.sql` and the
privileged wrapper upgrade. The first upgrade
is valid before the versioned public implementation exists because its PL/pgSQL
body resolves that implementation only when invoked; traffic must remain
stopped until the complete sequence finishes. Both operator scripts are
idempotent. Ordinary schema migrations must never replace functions owned by
the NOLOGIN bootstrap role:

```bash
psql "$STEWARD_ADMIN_DATABASE_URL" \
  -v steward_migration_role=steward_migrator \
  -v steward_bootstrap_role=steward_bootstrap_owner \
  -f scripts/postgres/rls-prepare-personal-lifecycle.sql

psql "$STEWARD_ADMIN_DATABASE_URL" \
  -v steward_app_role=steward_app \
  -v steward_migration_role=steward_migrator \
  -v steward_bootstrap_role=steward_bootstrap_owner \
  -f scripts/postgres/rls-upgrade-personal-lifecycle.sql
```

Migration `0119` also stores the monotonic `users.tokens_revoked_before`
boundary. Deactivation and reactivation advance it in the same audited database
transaction; request authentication reads it through the narrow
`steward_bootstrap.user_token_revocation_subject(uuid)` wrapper. Redis is only
an acceleration cache and a failed cache refresh cannot make an old token valid
again. Any exact bootstrap-function allowlist must include that read-only wrapper
for the app and its public implementation only for the bootstrap owner; it must
continue to exclude both lifecycle mutation wrappers from the app role.
...
`STEWARD_BOOTSTRAP_DATABASE_ROLE` to the exact non-login definer owner, and
`STEWARD_MIGRATION_DATABASE_ROLE` to the exact protected-relation owner. Set
`STEWARD_PLATFORM_DATABASE_URL`/`STEWARD_PLATFORM_DATABASE_ROLE` to the
...
relation/partition graph, policy definitions, and checked-in privileged
function semantics (identity, result, language, volatility, parallelism, body
digest, settings, owner, and exact app/platform/PUBLIC ACL) before
...
The two migration authorities are intentionally different. Core migrations
run through `STEWARD_ADMIN_DATABASE_URL`: later core releases may replace
functions owned by the non-login bootstrap role and therefore require the
database owner/operator authority. Plugin migrations run only through
`MIGRATION_DATABASE_URL` as the restricted `steward_migrator`; that login owns
protected relations for maintenance but cannot replace bootstrap-definer
functions or assume the bootstrap owner. Never point the core runner at the
restricted plugin URL, and never give the plugin runner the admin URL.
