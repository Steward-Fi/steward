...
The personal-lifecycle rollout has an explicit split-role sequence. Apply
`0113` as the restricted migration role, run the privileged wrapper upgrade as
the admin before `0114`, apply `0114` as the restricted migration role, then
rerun `rls-bootstrap.sql` and the privileged wrapper upgrade. The first upgrade
