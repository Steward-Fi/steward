...
- pure global platform identity operations still need a truthful reserved audit
  tenant/context strategy; tenant-addressed operations and agent-id token
  revocation are now derived and bound before their protected handlers;
- the newly propagated platform/proxy/background paths still need final
  deployment-role real-Postgres route/job proofs covering every production path.
...
fixed-shape bootstrap functions. Those functions reduce the remaining surface;
they do not waive either activation blocker above.
