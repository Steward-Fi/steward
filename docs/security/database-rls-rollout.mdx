...
The remaining activation blockers are explicit and must not be waived:

- `/auth` refresh lookup now uses a fixed bootstrap function, while refresh
  writes, membership links, identity claims, and personal-tenant wallet
  provisioning run after verified identity in tenant transactions. Remaining
  pre-context tenant/join/SSO discovery and cross-tenant refresh switching need
  fixed-shape bootstrap/transfer functions;
- platform global-user/identity routes and agent-id-only token revocation still
  need a tenant derivation or narrowly shaped cross-tenant bootstrap API;
- global deactivated-user retention still needs a fixed maintenance function
  that checks ownership across every tenant before deletion;
- the newly propagated platform/proxy/background paths still need final
  deployment-role real-Postgres route/job proofs.

Until those paths are complete, do not run `rls-activate.sql` in production.
...
Create/finalize roles after migration `0110` (the invoking operator needs
