...
- `blockedPatterns: string[]` — each is a bounded safe-subset regex; a match on the text
...
Policy regexes deliberately exclude grouping, alternation, lookarounds, and
backreferences, and permit at most one variable repetition. Unanchored
`blockedPatterns` with a variable repetition are rejected; prefix them with `^`
when repetition is required. This fail-closed subset prevents JavaScript regex
backtracking from blocking the API event loop.
