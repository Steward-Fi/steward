...
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_VERIFY_SERVICE_SID` / `TWILIO_VERIFY_TOKEN_TTL_SECONDS` | recommended managed SMS OTP path. Verify owns compliant delivery plus code checking; configure the Service for six digits. The TTL is required, must exactly match the Verify Service token-validity setting, and must be 120-86400 seconds (staging: `600`). Steward uses that value for purpose fencing and preserves Verify's authoritative `date_created` expiry across resends. `TWILIO_FROM` remains a legacy fallback and must name a sender whose registration is actually approved | `packages/auth/src/sms-provider.ts`, `packages/api/src/routes/auth.ts`, `.env.example` |
