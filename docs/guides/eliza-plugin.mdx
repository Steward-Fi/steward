...
STEWARD_JWT=stwd_jwt_...                   # Agent JWT (optional, preferred over API key)
```
...
ENV STEWARD_JWT=stwd_jwt_...
ENV STEWARD_AGENT_ID=my-agent
```

When using the [Proxy Gateway](/concepts/proxy-gateway), add its URL to the core
plugin configuration above and use an agent JWT:
