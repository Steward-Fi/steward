...
description: "Deterministic validation and the separate live GitHub sandbox run for governed-provider execution."
---

# Provider Authority: Golden Path

This workflow validates the governed-provider contract with a
**single-command deterministic CI run** that executes the whole matrix against a
...
```
POST /v2/provider-actions          (create; action-creation canonicalization + binding)
  -> access check                  (provider-operation grants/role-bindings)
  -> policy decision               (allow / approval_required)
  -> POST /:id/approval            (human approve/deny with typed reason; approval-lifecycle)
  -> POST /:id/execute             (typed system resume; approval-lifecycle mints the execution-authorization nonce)
  -> dispatchGovernedExecution     (atomic single-winner claim; execution-authorization)
...
No new authority is introduced by the golden path. It exercises provider-operation–evidence only.
...
## 1. Deterministic CI validation (credential-free)
...
CLI commands are convenience wrappers over the same action-creation–evidence routes; CLI
