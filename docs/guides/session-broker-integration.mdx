...
| `constraints.argMatches` | object | Each key must match the full value using Steward's bounded safe regex subset (no groups, alternation, lookarounds, or backreferences; at most one variable repetition). |
