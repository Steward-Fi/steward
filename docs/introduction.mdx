...
description: "policy-gated wallet and credential infrastructure for autonomous AI agents. encrypted wallets, credential management, audit trail, embeddable React UI."
---

Steward is a governance layer for autonomous AI agents. it provides
encrypted wallet management, credential storage, route-level policy gates, API
...
supported API calls and transactions flow through Steward routes, where they are authenticated, policy-checked, logged, and metered before Vault signing or proxy credential injection happens.
