...
| `STEWARD_REQUEST_SIGNING_SECRET` / `STEWARD_REQUEST_SIGNING_SECRETS` | Production machine mutation signatures cannot be verified after restoring different roots. Preserve the active and overlapping verification roots for the longest accepted request window; keep them distinct from proxy, JWT, audit, and master secrets. |
