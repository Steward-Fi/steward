...
| `POSTGRES_PASSWORD` and the credentials represented by `DATABASE_URL`, `STEWARD_ADMIN_DATABASE_URL`, `MIGRATION_DATABASE_URL`, and `STEWARD_PLATFORM_DATABASE_URL` | Required for the distinct app, administrator, migration, and platform-authority database identities. They may be reset by a database administrator without changing encrypted application data, but all four authority URLs must be restored consistently to Compose/application configuration. Do not put passwords in command history. |
