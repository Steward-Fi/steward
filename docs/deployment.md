...
`/health` is a liveness check. `/ready` verifies migrations have completed, the database is reachable, and `STEWARD_MASTER_PASSWORD` is set. When the capabilities plugin is enabled it also verifies the capability rate-limit backend: healthy configured Redis, or an actual tenant-bound, forced-RLS PostgreSQL DML exercise (including the bucket writer trigger, tenant-deletion lock function, and parent-agent lookup) when Redis is absent. The exercise rolls back inside a PL/pgSQL exception subtransaction and leaves no readiness row. Process memory is reported as ready only for explicit `NODE_ENV=development|test`; configured-but-unavailable Redis and a missing, inaccessible, or incorrectly protected production bucket are not ready. Unauthenticated `/ready` responses contain only the status code and per-check `ok` flags; set `STEWARD_READY_PROBE_TOKEN` and send it as the `X-Steward-Probe-Token` header to receive the full diagnostic detail (migration tags, backend identity, clock skew).
...
| `REDIS_URL` | Redis for the production global limiter, token/challenge stores, proxy and adapter spend tracking/cache | none | Required for production global request limiting unless a guaranteed single-instance Bun deployment uses the explicit acknowledgement below. Production adapter daily-cap checks deny without available Redis. API auth stores may use Postgres; the production proxy has its own fail-closed/soft-fail contract. |
...
| `STEWARD_ADAPTER_PER_OP_CAP_USD` | Per-operation USD cap for unsigned adapter swap/earn/bridge intents | `10000` | Must be a positive finite number. |
| `STEWARD_ADAPTER_DAILY_CAP_USD` | Rolling daily potential-spend cap for unsigned adapter swap/earn/bridge issuance | `50000` | Production atomically reserves each canonical request by `Idempotency-Key` in durable Redis before returning an artifact and denies when Redis is absent or down; explicit development/test posture remains permissive without Redis. |
...
| `SKIP_MIGRATIONS` | Confirm out-of-band production migrations/activation | required in production | Set `true` or `1`; production refuses automatic migrations and validates the restricted role plus activated RLS catalog. |
