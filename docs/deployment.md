...
| `STEWARD_ADAPTER_DAILY_CAP_USD` | Rolling daily USD cap for unsigned adapter swap/earn/bridge intents | `50000` | Admission atomically reserves settled plus in-flight budget in Redis. Missing/down Redis denies except for explicitly acknowledged development/test memory mode. |
