...
| `STEWARD_ADAPTER_DAILY_CAP_USD` | Rolling daily USD cap for unsigned adapter swap/earn/bridge intents | `50000` | Production enforcement requires available durable Redis spend state and denies when it is absent or down; explicit development/test posture remains permissive without Redis. |
