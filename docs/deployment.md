...
| `STEWARD_ALLOW_MEMORY_TRADING_IDEMPOTENCY` | Explicit single-instance acknowledgement for restart-unsafe trading idempotency | none | Never recommended. Production fund-moving trading routes require durable Redis idempotency unless this is exactly `true`; a restart loses replay protection. |
