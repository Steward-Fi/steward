...
| `REDIS_URL` | Redis for the production global limiter, token/challenge stores, proxy spend tracking/cache | none | Required for production global request limiting unless a guaranteed single-instance Bun deployment uses the explicit acknowledgement below. API auth stores may use Postgres; the production proxy has its own fail-closed/soft-fail contract. |
