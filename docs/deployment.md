...
| `STEWARD_KDF_SALT` | Deployment salt for scrypt master-key derivation | legacy built-in salt outside production | Required in production; must be valid hex encoding at least 16 bytes (32 hex characters). Back it up with the encrypted vault data. |
...
| `STEWARD_DEFAULT_TENANT_ID` | Tenant used by auth routes when a request supplies neither `X-Steward-Tenant` nor a body `tenantId` | `default` | The tenant must already exist. Workers resolve this from the active request binding, so configure it per deployment/environment. |
...
| `STEWARD_NATIVE_PRICE_FALLBACK_USD` | Conservative USD price per native EVM token when the price oracle is unavailable during spend recording | `10000` | Must be a positive finite number. Workers resolve it from the active request binding. |
...
| `REDIS_URL` | Redis for the production global limiter, token/challenge stores, proxy and adapter spend tracking/cache | none | Required for production global request limiting unless a guaranteed single-instance Bun deployment uses the explicit acknowledgement below. Production daily-cap checks deny without available Redis for adapter swap/earn/bridge and fund-moving Spark intents. API auth stores may use Postgres; the production proxy has its own fail-closed/soft-fail contract. |
| `STEWARD_ACKNOWLEDGE_SINGLE_INSTANCE_GLOBAL_RATE_LIMIT` | Explicit single-instance acknowledgement for restart-resetting global request limits | none | Exact `true` permits production Bun to use bounded process memory when Redis is absent. Never enable for multi-replica deployments; Workers ignore it. |
| `STEWARD_ADAPTER_PER_OP_CAP_USD` | Per-operation USD cap for unsigned adapter swap/earn/bridge intents and the four fund-moving Spark intents | `10000` | The Spark routes are static-BTC deposit claim, Lightning pay, transfer, and token transfer. Must be a positive finite number. |
| `STEWARD_ADAPTER_DAILY_CAP_USD` | Rolling daily USD cap for unsigned adapter swap/earn/bridge intents and the four fund-moving Spark intents | `50000` | The Spark routes are static-BTC deposit claim, Lightning pay, transfer, and token transfer. Admission atomically reserves settled plus in-flight budget in Redis. Missing/down Redis denies except for explicitly acknowledged development/test memory mode. |
| `STEWARD_ALLOW_MEMORY_ADAPTER_SPEND` | Explicit development/test acknowledgement for process-local adapter spend reservations | none | Exact `true` is required without Redis and is ignored in production, Workers, staging/unknown postures. Restart and multi-replica safety require Redis. |
...
| `STEWARD_ALLOW_MEMORY_TRADING_RATE_LIMITS` | Explicit single-instance acknowledgement for process-local trading order and operator-transfer limits | none | Never enable for multi-instance production. Without Redis, production trading submissions and operator transfers fail closed unless exact `true` is set. |
...
| `STEWARD_BOOT_TIMEOUT_MS` / `STEWARD_STARTUP_PHASE_TIMEOUT_MS` | Overall Bun boot / individual startup phase deadline in milliseconds | `240000` / `60000` | Positive integers; expiry logs a redacted phase marker and exits before accepting traffic. |
| `STEWARD_MIGRATION_CONNECT_TIMEOUT_SECONDS` | PostgreSQL migration connection deadline in seconds | `10` | Positive integer. |
| `STEWARD_MIGRATION_LOCK_TIMEOUT_MS` / `STEWARD_MIGRATION_STATEMENT_TIMEOUT_MS` / `STEWARD_MIGRATION_OVERALL_TIMEOUT_MS` | Advisory-lock, statement, and overall migration deadlines in milliseconds | `30000` / `120000` / `180000` | Positive integers; timeout closes the migration connection so PostgreSQL rolls back open work. |
