...
| `REDIS_URL` | Redis for rate limiting, token/challenge stores, proxy spend tracking/cache | none | API auth stores may use Postgres, but the production proxy fails closed without Redis unless its explicit soft-fail override is enabled. |
