/**
 * provider-action-service.ts — the provider-action creation and authority pipeline.
 *
 ...
 * authorization, or performs network I/O. That is execution-authorization.
 */
 ...
 * exact identity so a known outcome can be reconciled after a
...
// ─── The in-process executor stub (action-creation only — NEVER real dispatch) ─────────────

/**
 * The action-creation executor stub. It accepts ONLY a persisted intent id and reloads the
 ...
 * credential-bound enforcement (that is execution-authorization).
 */
...
class ProviderActionService {
  ...
  async createProviderAction(input: CreateProviderActionInput): Promise<ProviderActionOutcome> {
    ...
    // Finalize a deferred generic-http build now that the operation
    ...
    // Hoisted so the catch/drain-failure paths can reclaim reservations
    ...
    try {
      await this.db().transaction(async (tx) => {
        ...
        // approval-lifecycle (§6.3): for the approval-required arm, build the exact approval
        // commitment + queue row inside THIS create transaction (AFTER the intent
        // insert the queue FK references). A missing provider-operation execution dependency
        ...
        // evidence C1: correlated lifecycle events set resource_type='provider_action'
        // and resource_id=intents.id (not the operation id) so evidence can correlate
        ...
        await tx.insert(providerActionAuditOutbox).values({
          ...
          resourceId: intentId,
          metadata: {
            // evidence C1: metadata.intentId is the offline-authoritative correlation
      ...
      // A missing provider-operation execution dependency (route/credential) fails approval
    ...
    if (!drained) {
      // The binding ALREADY committed. If its status is an
    ...
    if (effects.policy === "approval_required") {
      // Current limitation: a cumulativeSpend / maxCalls
      ...
      // Correct enforcement requires the approval-execute path in
      // provider-approval.ts to re-reserve at EXECUTION time. Until that invariant
      // exists, this branch deliberately avoids holding phantom budget.
      await this.reconcilePolicyReservations(tenantId, intentId);
      return {
        kind: "approval_required",
        code: "APPROVAL_REQUIRED",
    ...
    try {
      ...
      // OUTCOME_UNKNOWN: the stub threw AFTER we admitted and reserved; we
  ...
  private async reserveCumulativeSpendForInvoke(input: {
    ...
    // invoke is counted once and never double-counted). priorSums come back
  ...
  private async evaluatePolicy(args: {
    ...
    // operation's request profile. action-creation reads the enabled capability-intent rules
    ...
    try {
      ...
      // Atomically reserve one invoke slot against all configured count caps so
      // concurrent invokes cannot collectively exceed maxCalls.
      ...
      // One invoke is counted once across all count windows. Each window's count
      // is fed to the composer under its own bucket. A rejection on any window denies (no slot taken); a
      ...
      const context: ProviderPolicyContext = {
        ...
        // Trailing-hour count is not wired during action creation; rules that require it will
        // fail closed (POLICY_INPUT_UNAVAILABLE) exactly as specified.
        invokeCount1h: undefined,
        // Per-cap trailing-window counts are undefined when unwired, which makes
        // a maxCalls rule fail closed.
        ...(windowedInvokeCounts !== undefined ? { windowedInvokeCounts } : {}),
        ...(policyText !== undefined ? { policyText } : {}),
        // Cumulative-spend aggregate and declaration. spendDeclaration absent =>
        ...
        // and current minute) are not wired into the service, matching the
        // posture of invokeCount1h above. A permissioned-X rule that requires one
  ...
  async recoverUnsignedIntents(tenantId: string, intentId?: string): Promise<number> {
    ...
    // The same C2 pass covers durable policy reservations. Audit and
  ...
  private async outcomeFromBinding(
    ...
    // Approval-required lineage (approval-lifecycle). A create-replay of a governed provider
...
/**
 * Extract the operation's requester-separation requirement (spec I10). The current operation
 * contract stores approval requirements in
 * `request_profile.approvalRequirements.requesterSeparation` when present
 * and defaults to false when the field is absent.
 */
