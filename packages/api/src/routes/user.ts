...
user.patch("/me/tenants/:tenantId/users/:targetUserId/deactivate", async (c) => {
  ...
  const result = await withTenantAuditedTransaction(
    ...
    async (txRaw, appendRequiredAudit) => {
      ...
      const [lifecycle] = await tx.execute(sql<{ tokensRevokedBefore: number | string }>`
        SELECT tokens_revoked_before AS "tokensRevokedBefore"
        FROM steward_bootstrap.tenant_set_user_deactivation(
          ${tenantId}, ${admin.userId}::uuid, ${targetUserId}::uuid, ${deactivated}
        )
      `);
      if (!lifecycle) throw new Error("Tenant lifecycle target disappeared");
      const issuedBefore = Number(lifecycle.tokensRevokedBefore);
      await revocationStore.revokeUserTokens(targetUserId, issuedBefore);
      await tx
        .delete(refreshTokens)
        .where(and(eq(refreshTokens.tenantId, tenantId), eq(refreshTokens.userId, targetUserId)));
