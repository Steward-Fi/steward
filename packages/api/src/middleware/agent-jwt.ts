...
async function resolveVerifiedAgentJwt(
  ...
  // requested tenant. Tokens without the claim are still bound by the
  // tenant-to-agent registration below.
...
export async function requireCapabilityAgentJwt(
  ...
  await installAgentJwtContext(c, auth);
  // Deliberately outside the short authentication transaction. Individual
  // durable operations (including the rate reservation) open and commit their
  // own tenant transaction before any downstream/provider I/O.
  return next();
}
