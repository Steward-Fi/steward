...
async function tenantRequestSigningKeyCandidates(request: Request): Promise<string[]> {
  ...
  // This middleware runs before route authentication. Require one well-formed,
  // indexed key id before performing the expensive tenant-key KDF and decrypt.
  // Requests without a key id never evaluate tenant signing keys.
...
export function authorizationSignature(options?: AuthorizationSignatureOptions) {
  ...
  return createMiddleware<{ Variables: AppVariables }>(async (c, next) => {
    ...
    // A named, valid tenant key id is required before the tenant-key lookup and
    // KDF. Key-id-less requests use only static or app-secret candidates.
