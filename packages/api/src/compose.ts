 ...
 * core lean and the deploy configurable.
 *
...
export async function composeApp(): Promise<Hono<{ Variables: AppVariables }>> {
  ...
  // FULL MODE: register this deploy's enabled opt-in plugins with the required
  // registration order and migrations.
  //
...
/**
 * Apply this deploy's opt-in plugins' OWN migrations (plugin migration contribution contract), into per-plugin
