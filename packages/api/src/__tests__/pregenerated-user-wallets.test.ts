...
describe("pregenerated user wallet routes", () => {
  ...
  it("claims only hashed pregenerated wallets into personal user wallets", () => {
    ...
    expect(route).toContain("eq(agents.platformId, originalClaimPlatformId)");
    expect(route).toContain("pregeneratedWalletClaimLifecycles");
    ...
    expect(route).toContain("USER_WALLET_DEFAULT_POLICIES.map");
    expect(route).toContain("CLAIMED_PREGENERATED_CLAIM_PREFIX");
    expect(route).toContain("Claim source authority could not be retired");
