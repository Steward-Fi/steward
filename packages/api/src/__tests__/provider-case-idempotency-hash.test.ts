/**
 * Provider idempotency-key data-minimization guard (§3.4 / C3): the raw provider
...
process.env.STEWARD_MASTER_PASSWORD ||= "provider-case-idempotency-hash-master";
...
describe("provider idempotency key is hashed, never raw (§3.4)", () => {
  beforeAll(async () => {
