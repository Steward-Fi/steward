...
let membershipOwnsUserLock = false;
for (let attempt = 0; attempt < 100 && !membershipOwnsUserLock; attempt += 1) {
  membershipOwnsUserLock = !(await admin.begin(async (tx) => {
  ...
  if (!membershipOwnsUserLock) await Bun.sleep(20);
}
if (!membershipOwnsUserLock) {
  releaseTenantLock();
  await blocker;
  const response = await membership;
  throw new Error(
    `mounted membership route did not expose user-before-tenant ordering: ${response.status}:${await response.text()}`,
  );
}
...
await admin.end();
await closeDb();
console.log("mounted membership/lifecycle lock order passed");
process.exit(0);
