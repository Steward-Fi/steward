/**
 * X governed provider-action coverage for the authority plane.
 *
 * This suite proves that the binding profile constraint admits exactly the
 * GitHub and X canonical profiles, that X capability-intent policy composes to
 * `approval_required` for writes and `allow` for reads, and that the full X
 * governed chain works through the real provider-action and approval services:
 *
 *   connect (seed X vault secret + provider_accounts row directly) -> agent proposes x.tweet.create via the service
 ...
 * in-process stub; byte-level Bearer injection is proven separately on the proxy plane in
 ...
 *     it reuses the approval commitment's executionDependencies.secretVersion binding
...
// ─────────────────────────────────────────────────────────────────────────────
// The profile CHECK admits both supported canonical profiles and rejects every
// other value, proving an exact allowlist rather than a blanket relaxation.
// ─────────────────────────────────────────────────────────────────────────────
...
describe("0082 profile-CHECK widening: X admitted, unknown profiles still rejected", () => {
  ...
  test("an unknown profile is STILL rejected by provider_action_bindings_profile_chk (exact allowlist)", async () => {
    // Prove the allowlist is exact: a profile outside the {github, x} set is
...
// ─────────────────────────────────────────────────────────────────────────────
// Full X governed E2E through the persisted binding and approval chain.
// ─────────────────────────────────────────────────────────────────────────────

describe("X governed provider-action E2E", () => {
  ...
  test("NEGATIVE: tampered canonical action bytes after approval => digest mismatch, stales", async () => {
    ...
    // The binding guard (steward_provider_action_binding_guard) freezes
