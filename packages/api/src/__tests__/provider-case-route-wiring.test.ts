/**
 * Provider-case real-app route-wiring regression guard.
 *
 * The focused integration tests mount `providerCaseRoutes` on a mock Hono
 ...
 *
 * Regression guard for the required `tenantAuth` wiring on the case
...
describe("provider case routes: real composed-app auth wiring", () => {
  let app: any;
  ...
  beforeAll(async () => {
    ...
    await getDb()
      .insert(tenants)
      .values({ id: TENANT, name: "Provider Case Wiring", apiKeyHash: "provider-case-wiring" })
      .onConflictDoNothing();
    const [owner] = await getDb()
      .insert(users)
      .values({ email: "owner@provider-case.test", name: "Owner" })
      .returning();
    const [member] = await getDb()
      .insert(users)
      .values({ email: "member@provider-case.test", name: "Member" })
      .returning();
    await getDb()
      .insert(userTenants)
      .values([
