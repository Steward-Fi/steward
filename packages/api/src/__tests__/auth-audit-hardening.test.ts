...
describe("auth and audit hardening", () => {
  ...
  it("revokes already minted access tokens when refresh-token reuse is detected", () => {
    ...
    const reuseRevocation = authSource.indexOf("revokeUserRefreshSessions(", reusedBranch);
    expect(reuseRevocation).toBeGreaterThan(reusedBranch);
    expect(authSource.indexOf("rotatedRefresh.userId", reuseRevocation)).toBeGreaterThan(
      reuseRevocation,
    );
  ...
  it("fences refresh rotation against concurrent user session revocation", () => {
    ...
    expect(
      ...
      authSource.indexOf("steward_bootstrap.auth_rotate_refresh_token", rotationStart),
    );
    expect(
