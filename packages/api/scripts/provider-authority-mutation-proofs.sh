#!/usr/bin/env bash
# provider-authority mutation-strength proofs (spec §5.3). Each mutation weakens ONE security/
...
# be a no-op when the real value already satisfies it — the evidence convention is to
...
# available to @stwd/api (devDependency added in provider-authority).
set -uo pipefail
...
proof "1 seam: default forwarder must be forwardWithVettedDns (PN02)" \
  "$PROXY" "$INVENTORY_TEST" "forwarder rebindings are limited" "$PROXY_SRC" \
