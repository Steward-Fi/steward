#!/usr/bin/env bash
# End-to-end validation mutation-strength proofs (spec §5.3). Each mutation
# weakens one security or honesty predicate in production or source code; a proof is valid iff the named
# GUARD test PASSES clean AND FAILS after the mutation. Every mutated file is
# restored after each proof.
#
# IMPORTANT: mutations target the CODE THE TEST GUARDS (production source, docs,
# config), NOT the test's own assertions. Weakening a test assertion would
# be a no-op when the real value already satisfies it, so mutate the guarded
# predicate itself.
#
#   Run from the repo root:  bash packages/api/scripts/provider-authority-e2e-mutation-proofs.sh
#
# Requires built @stwd/shared and @stwd/redis packages, and @stwd/proxy available
# to @stwd/api.
set -uo pipefail
...
# Target (GUARDED) files — production source, docs, config.
...
# 4: inject an affirmative prohibited claim → the prohibited-claim scanner
#    (U8/PN38) must fail.
proof "4 claims: prohibited exactly-once/operator-proof claim rejected (U8/PN38)" \
...
# 5: drop the required exec-auth secret from compose → the compose-env
#     verification (G2/U10) must fail.
proof "5 compose: exec-auth secret required in enterprise-reference (G2/U10)" \
...
echo ""
echo "=== end-to-end validation mutation proofs: $pass_count killed / $((pass_count + fail_count)) total ==="
