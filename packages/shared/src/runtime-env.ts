...
/**
 * Bind an immutable environment snapshot to one asynchronous request. Missing
 * keys remain missing instead of falling through to another request's global
 * state. Outside a bound request, Bun retains its normal process.env behavior.
 */
...
/** Return the complete current snapshot for APIs that accept an env object. */
export function currentRuntimeEnvironment(): RuntimeEnvironment {
  return runtimeEnvironmentStorage.getStore() ?? process.env;
}

export function runtimeEnvironmentFlag(name: string): boolean {
  return runtimeEnvironmentValue(name) === "true";
}
