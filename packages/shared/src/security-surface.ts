...
export interface SecuritySurfaceRoute {
   ...
   * execution-authorization policy-bound execution gateway (GovernedVault mint+consume before raw
   * signing). `false`/undefined means the route reaches the raw signer through
   * route-local policy only, outside the execution gateway. This must
...
/**
 * A raw EVM `Vault.signTransaction` call site outside the execution-authorization
 * gateway. Enumerated explicitly so the security surface never over-claims
...
export const SECURITY_SURFACE_OPERATIONS = [
  {
    ...
    routes: [
      ...
      },
      {
        ...
        notes:
          "Transfer action surface. Route-local policy gated and outside the execution gateway.",
      },
      {
        file: "packages/api/src/routes/vault.ts",
        method: "POST",
      ...
      },
      {
        ...
        gatewayMigrated: false,
        notes:
          "Replace/RBF surface; route-local policy only; not gateway-migrated (pending gateway convergence).",
      },
      {
        ...
        gatewayMigrated: false,
        notes:
          "Legacy intent execution EVM sign; not gateway-migrated (pending gateway convergence).",
      },
      {
        ...
        gatewayMigrated: false,
        notes:
          "Personal user-session EVM sign; not gateway-migrated (pending gateway convergence).",
      },
      {
        ...
        path: "/rpc",
        notes:
          "eth_sendTransaction compatibility path; not gateway-migrated (pending gateway convergence).",
        gatewayMigrated: false,
      },
    ],
    auth: [
...
/**
 * Explicit enumeration of raw EVM `Vault.signTransaction` call sites outside
 * the execution-authorization gateway. The gateway-authorized primary EVM
 ...
 * route-local branches (transfer action surface); the intents/user/global-wallet
 * entries are separate route files retained in this explicit inventory.
 *
...
export const LEGACY_EVM_SIGN_CALL_SITES = [
  {
    ...
    reason:
      "Transfer action EVM sign. Route-local policy gated, separate surface from primary sign; gateway convergence.",
  },
  {
    file: "packages/api/src/routes/vault.ts",
    approxLine: 3794,
    ...
    reason:
      "Approval replay raw fallback for the TRANSFER surface only. An invariant guard throws if a primary-EVM raw-signing candidate reaches this branch; gateway convergence for transfers.",
  },
  {
    file: "packages/api/src/routes/intents.ts",
    approxLine: 596,
    path: "POST /:intentId/execute",
    reason: "Legacy intent execution EVM sign; gateway convergence.",
  },
  {
    file: "packages/api/src/routes/intents.ts",
    approxLine: 715,
  ...
  },
  {
    ...
    reason: "Personal user-session EVM sign; gateway convergence.",
  },
  {
    file: "packages/api/src/routes/global-wallet.ts",
    approxLine: 1540,
    path: "POST /rpc (eth_sendTransaction)",
    reason: "Global wallet compatibility EVM sign; gateway convergence.",
  },
 ...
 *    entry in LEGACY_EVM_SIGN_CALL_SITES (gateway convergence).
 */
...
export const RAW_EVM_SIGN_INVENTORY = [
  ...
  // ── packages/api/src/routes/intents.ts (2 raw calls) ──
  {
    ...
    classification: "legacy",
    reason:
      "Legacy intent execution EVM sign (single-transfer branch); gateway convergence. Anchored to its policy-rejection guard string.",
  },
  {
    file: "packages/api/src/routes/intents.ts",
    ...
    classification: "legacy",
    reason:
      "Legacy intent batch-call execution EVM sign; gateway convergence. Anchored to its batch policy-rejection guard string.",
  },
