 ...
 *
 * This module owns the governed-provider action-creation contract:
...
// ─────────────────────────────────────────────────────────────────────────────
// execution-authorization: provider execution authorization v2 commitment (spec §3)
//
...
// 13 in-house JCS). The commitment binds the EXACT outbound bytes, the EXACT approval-lifecycle
...
/**
 * sha256: hex of JCS of the sorted selected-header NAME set from an action-creation canonical
 ...
 * injected credential header is never among selectedHeaders (action-creation rule).
 */
...
// execution-authorization deterministic v2 commitment builder + grant-dependency hash + outbound
...
// from the SAME persisted approval commitment + canonical action.

/**
 * Deterministic grant/binding dependency hash. Binds the EXACT matched
 * grant/binding ids+revisions the approval-lifecycle access decision committed, so a revoked or
 ...
 * the leading `?`) using the action-creation RFC3986 encoding rule (uppercase percent hex,
...
/** Inputs to reconstruct a v2 commitment from the approval commitment. */
 ...
 * canonical action + mint params. Both the API mint and the proxy claim call
 ...
 * operation arguments, and what the execution-authorization proxy recomputation + the offline
...
export function canonicalizeRawInternalAction(raw: RawInternalAction): GithubCanonicalActionV1 {
  ...
  if (BODYLESS_METHODS.has(method)) {
    ...
    // DELETE: bodyless only in action-creation (no operation declares a DELETE body).
    if (hasBody || hasContentType) fail("CANON_BODY_FORBIDDEN", `${method} must not carry a body`);
  }
