/**
 * governed-execution.ts — execution-authorized governed dispatch entry (spec §2.3, §4.1, §5.3,
 ...
 * the MINT stays in the API inside the approval-lifecycle resume tx. The v2 signing crypto is in
 ...
 *
 * @param intentId the approval-lifecycle intent (lifecycle root).
...
export async function dispatchGovernedExecution(
  ...
  // be caught here (and again in the boundary check, defense in depth).
  {
    ...
    // Route↔operation binding: the nonce binds routeId and operationId
  ...
  try {
    await withTenantAuditedTransaction(tenantId, async (tx, appendRequiredAudit) => {
      ...
      // terminal from another path). The approval-lifecycle transition trigger requires
...
// NEVER the raw key — evidence C3 data minimization).
...
async function dispatchOnce(
  ...
  // canonical action digest + v2 signature. JSON.stringify does not
  ...
  // Drain/cancel the proxy response body on every governed path. The
...
async function recordTerminal(
  ...
  await withTenantAuditedTransaction(tenantId, async (tx, appendRequiredAudit) => {
    ...
    // by exactly 1 (approval-lifecycle trigger convention) and scope by the executing precursor.
    await dbTx
      .update(providerActionBindings)
      .set({
