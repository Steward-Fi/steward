/**
 * governed-execution governed proxy cutover — proxy-side claim/dispatch + authority gate.
 *
 * Covers the security-critical proxy invariants against PGLite with the existing
 * forwarder stub (the "test transport seam"; provider-authority owns the real sandbox):
...
describe("governed-execution governed proxy authority gate (X1, §5.1)", () => {
...
describe("governed-execution dispatchGovernedExecution claim + dispatch", () => {
  ...
  it("P12c: substituted nonce route revision cannot escape the signed approval tuple", async () => {
    ...
    // while leaving the signed approval-lifecycle approval commitment at revision 1. Comparing
