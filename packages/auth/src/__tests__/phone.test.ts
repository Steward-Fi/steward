...
class ManagedProviderDouble implements ManagedSmsOtpProvider {
  ...
  async send(phone: string, channel: ManagedOtpDeliveryChannel): Promise<{ expiresAt: Date }> {
    ...
    if (this.challengeExpiresAt === null || this.challengeExpiresAt <= Date.now()) {
...
class LeaseExpiringBeforeProviderBackend extends MemoryBackend {
  ...
  override async setIfNotExists(key: string, value: string, ttlMs: number): Promise<boolean> {
...
describe("PhoneAuth", () => {
  ...
  test("distinguishes a local preflight failure from an ambiguous provider outcome", async () => {
    ...
    try {
      ...
      const ambiguous = await auth.verifyOtp(phone, "123456", "login:tenant-a").then(
        () => null,
        (error: unknown) => error,
      );
  ...
  test("keeps another purpose blocked through provider validity after delayed acceptance", async () => {
    const provider = new ManagedProviderDouble();
    const auth = new PhoneAuth({ managedProvider: provider, tokenTtlMs: 5 * 60 * 1000 });
  ...
  test("rejects an approved check when its exact operation lease expired before release", async () => {
    ...
    try {
      ...
      const staleApproval = await auth.verifyOtp(phone, "123456", "login:tenant-a").then(
        () => null,
        (error: unknown) => error,
      );
      expect(staleApproval).toBeInstanceOf(SmsVerificationError);
      expect(staleApproval).not.toBeInstanceOf(SmsVerificationNotAttemptedError);
    } finally {
      auth.destroy();
    }
  });
