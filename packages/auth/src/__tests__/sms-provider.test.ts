...
describe("TwilioVerifyProvider", () => {
  test("starts an SMS verification without handling the OTP body locally", async () => {
    ...
    expect(delivery.expiresAt.getTime()).toBe(createdAt.getTime() + 10 * 60 * 1000);
  ...
  test("uses the authoritative status even when the deprecated valid field is absent", async () => {
    ...
    await expect(VERIFY_PROVIDER.verify("+14155550999", "123456")).resolves.toBe(true);
    await expect(VERIFY_PROVIDER.verify("+14155550999", "000000")).resolves.toBe(false);
  });

  test("preserves the original authoritative expiry on a resend", async () => {
    const createdAt = new Date(Date.now() - 1000);
    globalThis.fetch = (async () =>
      new Response(JSON.stringify({ status: "pending", date_created: createdAt.toISOString() }), {
        status: 201,
      })) as typeof fetch;
  ...
  test.each([
    ...
    86_401,
    600.5,
  ])("rejects a missing or invalid configured Verify TTL (%s)", (tokenTtlSeconds) => {
    expect(
      () =>
  ...
  test.each([
    -10 * 60 * 1000 - 1,
    60 * 1000,
  ])("rejects an expired or implausibly future challenge timestamp", async (createdAtOffsetMs) => {
  ...
  test("treats a missing or expired verification as an invalid code", async () => {
    ...
    await expect(VERIFY_PROVIDER.verify("+14155550999", "123456")).resolves.toBe(false);
  });
  ...
  test("redacts Verify send and check provider failures", async () => {
    ...
    const checkFailure = await VERIFY_PROVIDER.verify("+14155550999", "123456").then(
      () => null,
      (error: unknown) => error,
    );
    ...
    expect(warnings).toEqual([
      "[steward:auth] Twilio Verify send failed with status 400",
      "[steward:auth] Twilio Verify check failed with status 503",
    ]);
    for (const value of [
  ...
  test("maps transport failures to generic redacted provider errors", async () => {
    ...
    expect(warnings).toEqual([
      "[steward:auth] Twilio Verify send request failed",
      "[steward:auth] Twilio Verify check request failed",
    ]);
