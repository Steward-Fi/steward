...
export class PhoneAuth {
  ...
  constructor(config: PhoneAuthConfig = {}) {
    if (config.provider && config.managedProvider) {
      throw new Error("PhoneAuth accepts either provider or managedProvider, not both");
    }
    this.managedProvider = config.managedProvider;
    this.provider =
      config.provider ?? (config.managedProvider ? undefined : new ConsoleSmsProvider());
    ...
    if (
      ...
      config.managedProvider.reservationTtlMs <
        config.managedProvider.challengeTtlMs + 2 * config.managedProvider.operationLockTtlMs
    ) {
      throw new Error("managed provider reservation TTL must cover send and verify windows");
    }
    this.managedOperationLockTtlMs = config.managedProvider?.operationLockTtlMs ?? this.tokenTtlMs;
    this.managedReservationTtlMs = config.managedProvider?.reservationTtlMs ?? this.tokenTtlMs;
  ...
  private managedOperationError(
    ...
    if (kind === "verify" && error instanceof SmsVerificationError) return error;
    if (kind === "send") return new SmsDeliveryError();
    return providerAttempted ? new SmsVerificationError() : new SmsVerificationNotAttemptedError();
  }
  ...
  private async acquireManagedOperation(
    ...
    try {
      acquired = await this.tokenStore.setIfNotExists(key, token, this.managedOperationLockTtlMs);
    } catch (error) {
  ...
  private async releaseManagedOperation(lock: { key: string; token: string }): Promise<boolean> {
  ...
  private async withManagedOperation<T>(
    ...
    if (!released) throw this.managedOperationError(kind, undefined, providerAttempted);
    if (!outcome.ok) {
  ...
  private async reserveManagedChallenge(phone: string, purpose: string): Promise<string> {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (await this.tokenStore.setIfNotExists(key, initial, this.managedReservationTtlMs)) {
        return initial;
      }
      ...
      if (reservation.purposeHash !== purposeHash) throw new SmsChallengeInProgressError();
      const desired = newManagedChallengeReservation(purpose, reservation.challengeExpiresAt);
      if (await this.tokenStore.transition(key, current, desired, this.managedReservationTtlMs)) {
        return desired;
      }
    }
  ...
  async sendOtp(phone: string, purpose = "login"): Promise<{ expiresAt: Date }> {
    ...
    if (this.managedProvider) {
      return this.withManagedOperation(phone, "send", async (lock, markProviderAttempted) => {
  ...
  async verifyOtp(
    ...
    if (this.managedProvider) {
      return this.withManagedOperation(phone, "verify", async (lock, markProviderAttempted) => {
        const key = managedChallengeStorageKey(phone);
        const serializedReservation = await this.tokenStore.verify(key);
        const reservation = parseManagedChallengeReservation(serializedReservation);
        if (
        ...
        markProviderAttempted();
        if (!(await this.managedProvider!.verify(phone, code))) return { valid: false };
        ...
        if ((await this.tokenStore.consume(key)) !== claimed) return { valid: false };
