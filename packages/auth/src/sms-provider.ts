...
async function readTwilioVerifyResponse(response: Response): Promise<TwilioVerifyResponse | null> {
  try {
...
export class TwilioVerifyProvider implements ManagedSmsOtpProvider {
  ...
  constructor(config: TwilioVerifyProviderConfig) {
    if (!config.accountSid || !config.authToken) {
      throw new Error("TwilioVerifyProvider: accountSid and authToken are required");
    }
    if (!/^VA[0-9a-fA-F]{32}$/.test(config.serviceSid)) {
      throw new Error("TwilioVerifyProvider: a valid Verify Service SID is required");
    }
    if (
      ...
      throw new Error("TwilioVerifyProvider: tokenTtlSeconds must be between 120 and 86400");
    }
    const requestTimeoutMs = config.requestTimeoutMs ?? TWILIO_VERIFY_DEFAULT_REQUEST_TIMEOUT_MS;
    if (
      !Number.isSafeInteger(requestTimeoutMs) ||
      requestTimeoutMs <= 0 ||
      requestTimeoutMs > TWILIO_VERIFY_MAX_REQUEST_TIMEOUT_MS
    ) {
      throw new Error("TwilioVerifyProvider: requestTimeoutMs must be between 1 and 60000");
    }
    this.accountSid = config.accountSid;
    this.authToken = config.authToken;
    ...
    this.operationLockTtlMs = this.requestTimeoutMs + TWILIO_VERIFY_CLOCK_SKEW_MS;
  ...
  async send(to: string): Promise<{ expiresAt: Date }> {
    ...
    try {
      ...
      console.warn("[steward:auth] Twilio Verify send request failed");
      throw new SmsDeliveryError();
    }
    if (!response.ok) {
      console.warn(`[steward:auth] Twilio Verify send failed with status ${response.status}`);
      throw new SmsDeliveryError();
    }

    const payload = await readTwilioVerifyResponse(response);
    const createdAtMs =
      typeof payload?.date_created === "string" ? Date.parse(payload.date_created) : Number.NaN;
    if (
      payload?.status !== "pending" ||
      !Number.isFinite(createdAtMs) ||
      createdAtMs > Date.now() + TWILIO_VERIFY_CLOCK_SKEW_MS
    ) {
      console.warn("[steward:auth] Twilio Verify send returned an unexpected response");
      throw new SmsDeliveryError();
    }
    const expiresAtMs = createdAtMs + this.challengeTtlMs;
    if (!Number.isSafeInteger(expiresAtMs) || expiresAtMs <= Date.now()) {
      console.warn("[steward:auth] Twilio Verify send returned an expired challenge");
      throw new SmsDeliveryError();
    }
  ...
  async verify(to: string, code: string): Promise<boolean> {
    ...
    try {
      ...
      console.warn("[steward:auth] Twilio Verify check request failed");
    ...
    if (!response.ok) {
      console.warn(`[steward:auth] Twilio Verify check failed with status ${response.status}`);
      throw new SmsVerificationError();
    }

    const payload = await readTwilioVerifyResponse(response);
    if (!payload || typeof payload.status !== "string") {
      console.warn("[steward:auth] Twilio Verify check returned an unexpected response");
      throw new SmsVerificationError();
    }
