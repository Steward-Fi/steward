...
| `wallet` | **broker** | Signing authority must never leave custody. The agent requests a signature; Steward (vault or threshold signer) performs it and returns only the signature. There is no "scoped short-lived private key" to hand out. |
