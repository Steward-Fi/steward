...
export function createTradeRoutes(ctx: StewardAppContext): Hono<{ Variables: AppVariables }> {
  ...
  function hyperliquidClientOrderId(
    tenantId: string,
    agentId: string,
    idempotencyKey: string,
  ): `0x${string}` {
    return `0x${sha256(`hyperliquid:${tenantId.length}:${tenantId}:${agentId.length}:${agentId}:${idempotencyKey}`).slice(0, 32)}`;
  }
  ...
  async function findDurableHyperliquidOutcome(
    ...
    idempotencyKey: string,
    bodyHash: string,
  ): Promise<{ conflict?: true; response?: TradeIdempotencyResponse }> {
  ...
  async function claimDurableHyperliquidExecution(
    ...
    const envelope: TradeIdempotencyResponse = {
      ...
      body: {
        ok: false,
        error: "Trade execution requires reconciliation",
        data: {
          executionId,
  ...
  async function enforceOrderRateLimit(
    ...
    if (redis) {
      const result = await checkRateLimit(
        `ratelimit:trade:hyperliquid:${agentId}:1000`,
        1000,
        10,
        redis,
      );
  ...
  type RouteIdempotency = {
    ...
    store?: (response: TradeIdempotencyResponse) => Promise<boolean>;
  ...
  async function completeTradeIdempotencyBestEffort(
    ...
    try {
      return (await idempotency.store?.(envelope)) ?? false;
    } catch {
  ...
  tradeRoutes.post("/hyperliquid/order", async (c) => {
    ...
    const durableRequestBodyHash = hashBody({ ...body, idempotencyKey: undefined });

    let idempotency = await getIdempotency(tenantId, agentId, idempotencyKey, body);
    ...
    const order: HyperliquidOrder = {
      ...
      cloid: hyperliquidClientOrderId(tenantId, agentId, idempotencyKey),
    };
