...
export type StewardReleaseReadinessClient = StewardCoreRepairClient & StewardSchemaMigrationClient;
...
export async function inspectStewardReleaseReadiness(
  ...
  const client =
    options.client ?? (createPostgresClient() as unknown as StewardReleaseReadinessClient);
  try {
