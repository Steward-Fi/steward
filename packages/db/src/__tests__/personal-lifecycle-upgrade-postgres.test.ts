...
describeWithPostgres("personal lifecycle production upgrade topology", () => {
  ...
  test("upgrades privileged functions, preserves evidence, and serializes both race winners", async () => {
    ...
    expect(applied).toContain("0123_personal_lifecycle_invariants");
    await command(operatorArgs("scripts/postgres/rls-bootstrap.sql"));
    await command(operatorArgs("scripts/postgres/rls-upgrade-personal-lifecycle.sql"));
    await command(operatorArgs("scripts/postgres/rls-upgrade-personal-lifecycle.sql"));
    ...
    await expectDatabaseRejection(
      appWriter.begin(async (tx) => {
        await tx`SELECT set_config('steward.tenant_id', ${teamTenantId}, true)`;
        await tx`SELECT set_config('steward.user_id', ${teamTargetId}, true)`;
        await tx`SELECT * FROM steward_bootstrap.tenant_set_user_deactivation(
          ${teamTenantId}, ${teamAdminId}::uuid, ${teamTargetId}::uuid, true
        )`;
      }),
      /tenant lifecycle authority denied/,
    );
    await expectDatabaseRejection(
      appWriter.begin(async (tx) => {
        await tx`SELECT set_config('steward.tenant_id', ${teamTenantId}, true)`;
        await tx`SELECT set_config('steward.user_id', '', true)`;
        await tx`SELECT * FROM steward_bootstrap.tenant_set_user_deactivation(
          ${teamTenantId}, ${teamAdminId}::uuid, ${teamTargetId}::uuid, true
        )`;
      }),
    ...
    expect(await db`SELECT deactivated_at FROM users WHERE id = ${teamTargetId}`).toEqual([
      { deactivated_at: null },
    ]);
