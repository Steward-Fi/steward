...
SELECT format(
  'REVOKE EXECUTE ON FUNCTION steward_bootstrap.platform_user_tenant_ids(uuid) FROM %I',
  :'steward_app_role'
) \gexec
...
CREATE OR REPLACE FUNCTION steward_bootstrap.tenant_set_user_deactivation(
  p_tenant_id text,
  p_actor_id uuid,
  p_user_id uuid,
  p_deactivated boolean
)
RETURNS TABLE (user_id uuid, deactivated_at timestamptz, tokens_revoked_before bigint)
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = pg_catalog
AS $$
DECLARE
  actor_role text;
  target_role text;
  other_tenant_count bigint;
  other_active_owner_count bigint;
  authoritative_issued_before bigint;
BEGIN
  IF NULLIF(current_setting('steward.tenant_id', true), '') IS DISTINCT FROM p_tenant_id
     OR NULLIF(current_setting('steward.user_id', true), '') IS DISTINCT FROM p_actor_id::text
  ...
  IF actor_role IS NULL OR actor_role NOT IN ('owner', 'admin') THEN
    RAISE EXCEPTION 'tenant admin access required' USING ERRCODE = '42501';
  END IF;
  SELECT membership.role INTO target_role
  FROM public.user_tenants membership
  WHERE membership.tenant_id = p_tenant_id AND membership.user_id = p_user_id;
  IF target_role IS NULL THEN
    RAISE EXCEPTION 'user not found in tenant' USING ERRCODE = 'P0002';
  END IF;
  SELECT count(*) INTO other_tenant_count
  FROM public.user_tenants membership
  WHERE membership.user_id = p_user_id
    AND membership.tenant_id <> p_tenant_id
    AND membership.tenant_id <> 'personal-' || p_user_id::text;
  IF other_tenant_count > 0 THEN
    RAISE EXCEPTION 'tenant dashboard lifecycle changes are limited to users without other tenant memberships'
      USING ERRCODE = '23514';
  END IF;
  IF p_deactivated AND target_role = 'owner' THEN
    SELECT count(*) INTO other_active_owner_count
    FROM public.user_tenants membership
    JOIN public.users owner_user ON owner_user.id = membership.user_id
    WHERE membership.tenant_id = p_tenant_id
      AND membership.role = 'owner'
      AND membership.user_id <> p_user_id
      AND owner_user.deactivated_at IS NULL;
    IF other_active_owner_count < 1 THEN
      RAISE EXCEPTION 'cannot deactivate the sole owner' USING ERRCODE = '23514';
    END IF;
  END IF;
  authoritative_issued_before := floor(extract(epoch FROM clock_timestamp()))::bigint + 1;
  RETURN QUERY
  ...
  SET deactivated_at = CASE WHEN p_deactivated THEN now() ELSE NULL END,
      tokens_revoked_before = GREATEST(
        target.tokens_revoked_before, authoritative_issued_before
      ),
      updated_at = now()
  WHERE target.id = p_user_id
  RETURNING target.id, target.deactivated_at, target.tokens_revoked_before;
END
$$;
REVOKE ALL ON FUNCTION steward_bootstrap.tenant_set_user_deactivation(text,uuid,uuid,boolean)
FROM PUBLIC;
SELECT format(
