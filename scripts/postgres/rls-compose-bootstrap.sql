...
\getenv steward_platform_password STEWARD_DB_PLATFORM_PASSWORD
...
\if :steward_set_role_passwords
  SELECT length(:'steward_app_password') >= 24 AS app_password_valid,
         ...
         length(:'steward_platform_password') >= 24 AS platform_password_valid
  \gset
  ...
  \else
    ...
    \quit 1
  \endif
  \if :platform_password_valid
  \else
    \echo 'STEWARD_DB_PLATFORM_PASSWORD must contain at least 24 characters' >&2
    \quit 1
  \endif
\endif
