...
/**
 * Verify a completed out-of-band release through the same restricted database
 * role the API will use. This is intentionally read-only: privileged release
 * credentials are consumed by the preceding migrator/bootstrap steps and are
 * never used to make an API container appear ready.
 */
